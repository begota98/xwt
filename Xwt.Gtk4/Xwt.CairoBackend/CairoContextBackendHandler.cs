...
namespace Xwt.CairoBackend
{
	...
	public class CairoContextBackendHandler: ContextBackendHandler
	{
		...
		public override void DrawTextLayout (object backend, TextLayout layout, double x, double y)
		{
			...
			if (layout.Height <= 0) {
				Functions.UpdateLayout (ctx.Context, pl);
				Functions.ShowLayout (ctx.Context, pl);
			} else {
				var lc = pl.GetLineCount ();
				var scale = Pango.Constants.SCALE;
				double h = 0;
				var fe = new Cairo.FontExtents ();
				ctx.Context.FontExtents (out fe);
				var baseline = fe.Ascent / (fe.Ascent + fe.Descent);
				for (int i = 0; i < lc; i++) {
					var line = pl.GetLine (i);
					var ext = new Pango.Rectangle ();
					var extl = new Pango.Rectangle ();
					line.GetExtents (out ext, out extl);
					h += h == 0 ? (extl.Height / scale * baseline) : (extl.Height / scale);
					if (h > layout.Height)
						break;
					ctx.Context.MoveTo (x, y + h);
					Functions.ShowLayoutLine (ctx.Context, line);
				}
			}
		}
		...
		{
			CairoContextBackend gc = (CairoContextBackend)backend;
			var t = new Cairo.Matrix ();
			t.Init (m.M11, m.M12, m.M21, m.M22, m.OffsetX, m.OffsetY);
		...
		public override Matrix GetCTM (object backend)
		{
			...
			double x2 = 0;
			double y2 = 1;
			t.TransformPoint (ref x0, ref y0);
			t.TransformPoint (ref x1, ref y1);
			t.TransformPoint (ref x2, ref y2);
			...
			return new Matrix (xx, yx, xy, yy, x0 - cb.Origin.X, y0 - cb.Origin.Y);
		}

		public override object CreatePath ()
		{
			Cairo.Surface sf = new Cairo.ImageSurface (Cairo.Format.A1, 1, 1);
